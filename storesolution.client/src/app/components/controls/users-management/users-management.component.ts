...
export class UsersManagementComponent implements OnInit {
  ...
  onDataLoadFailed(error: HttpErrorResponse) {
    ...
    this.alertService.showStickyMessage('Erro de carregamento',
      `Não é possível recuperar usuários do servidor.\r\nErro: "${Utilities.getHttpResponseMessage(error)}"`,
  ...
  openUserEditor() {
    ...
    modalRef.shown.subscribe(() => {
      if (!this.userEditor)
        throw new Error('O componente do editor de usuário não foi definido.');
  ...
  deleteUser(row: UserEdit) {
    this.alertService.showDialog(`Tem certeza de que deseja excluir "${row.userName}"?`,
