...
export class RolesManagementComponent implements OnInit {
      ...
      .subscribe({
        ...
        error: error => {
          ...
          this.alertService.showStickyMessage('Erro de carregamento',
            `Não é possível recuperar funções do servidor.\r\nErro: "${Utilities.getHttpResponseMessage(error)}"`,
  ...
  openRoleEditor() {
    ...
    modalRef.shown.subscribe(() => {
      if (!this.roleEditor)
        throw new Error('O componente do editor de função não foi definido.');
  ...
  deleteRole(row: Role) {
    this.alertService.showDialog(`Tem certeza de que deseja excluir a "${row.name}" função?`,
      ...
      .subscribe({
        ...
        error: error => {
          ...
          this.alertService.showStickyMessage('Erro de exclusão',
            `Ocorreu um erro ao excluir a função.\r\nErro: "${Utilities.getHttpResponseMessage(error)}"`,
            MessageSeverity.error, error);
        }
      });
  }
