...
namespace StoreSolution.Core.Services.Account
{
    public class UserAccountService : IUserAccountService
    {
        ...
        public UserAccountService(StoreSolutionDbContext context, UserManager<ApplicationUser> userManager)
        {
            _context = context;
        ...
        public async Task<List<(ApplicationUser User, string[] Roles)>> GetUsersAndRolesAsync(int page, int pageSize)
        {
            ...
            var roles = await _context.Roles.Where(r => userRoleIds.Contains(r.Id)).ToArrayAsync();

            return users.Select(u => (u, roles.Where(r => u.Roles
                        .Select(ur => ur.RoleId)
                        .Contains(r.Id)).Select(r => r.Name!)
                        .ToArray())).ToList();
        }
