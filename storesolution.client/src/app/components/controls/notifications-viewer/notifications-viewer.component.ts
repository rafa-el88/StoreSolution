...
export class NotificationsViewerComponent implements OnInit, OnDestroy {
    ...
    this.dataLoadingSubscription = (isViewOnly ?
      ...
      .subscribe({
        ...
        error: error => {
          ...
          this.alertService.showMessage('Erro de carregamento', 'Falha ao carregar novas notificações do servidor!', MessageSeverity.warn);
          this.alertService.logError(error);

          if (this.dataLoadingConsecutiveFailurs++ < 5) {
            setTimeout(() => this.initDataLoading(), 5000);
          } else {
            this.alertService.showStickyMessage('Erro de carregamento', 'Falha ao carregar novas notificações do servidor!', MessageSeverity.error);
          }
        }
      });
  }
  ...
  deleteNotification(row: Notification) {
    this.alertService.showDialog(`Tem certeza de que deseja excluir a notificação "${row.header}"?`,
      DialogType.confirm, () => this.deleteNotificationHelper(row));
  }

  deleteNotificationHelper(row: Notification) {
    this.alertService.startLoadingMessage('Excluindo...');
    this.loadingIndicator = true;
    ...
    this.notificationService.deleteNotification(row)
      .subscribe({
        ...
        error: error => {
          ...
          this.alertService.showStickyMessage('Erro de exclusão',
    ...
    this.notificationService.pinUnpinNotification(row, pin)
      .subscribe({
        ...
        error: error => {
          ...
          this.alertService.showStickyMessage(opText + ' Erro',
            `Ocorreu um erro enquanto ${opText} a notificação.\r\nErro: "${Utilities.getHttpResponseMessage(error)}"`,
            MessageSeverity.error, error);
        }
      });
  }
